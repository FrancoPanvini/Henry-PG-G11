      ...
      <div className='fixed inset-0 z-50 overflow-y-scroll '>
        <div className='panel relative top-32 mx-auto w-9/12 h-auto p-6 bg-gradient-to-r from-fourtyLight to-fourtyDark'>
          ...
          {Object.keys(event).length === 0 ? (
            ...
            <>
              <div>
                <h3 className='font-bold text-4xl py-3 px-6 text-center text-white capitalize'>{event.name}</h3>
                <h3 className='font-bold text-2xl py-3 px-6 text-center text-white'>Organizado por {event.organizer}</h3>
              </div>
              <div className='flex justify-center items-center h-full'>
                <div className={event.photo ? 'w-1/2 pr-6' : 'w-full'}>
                  ...
                  <p className=' text-lg p-6 text-justify text-white italic border-b-2 border-fourty border-opacity-25'>
                    ...
                    <span className='capitalize'>{`${new Date(event.initDate).toLocaleString().slice(0, -3)}`}</span>
                    <br />
                    <b>Fecha de fin:</b> <br />
                    <span className='capitalize'>{`${new Date(event.endDate).toLocaleString().slice(0, -3)}`}</span>
                  </p>
                ...
                <div className='w-1/2 px-6 py-4 rounded-xl shadow-inner bg-fourtyDark bg-opacity-20'>
                  <div className='h-full w-full flex justify-center'>
                    <img
                      src={event.photo ? event.photo : 'https://drpp-ny.org/wp-content/uploads/2014/07/sorry-image-not-available.png'}
                      alt='not available'
