...
function PhotoMiniSlider({ photos, setUrl }) {
  const renderArrows = ({ type, onClick, isEdge }) => {
    ...
    return (
      <div onClick={onClick} disabled={isEdge} className={`flex items-center justify-center rounded-md bg-thirtyLight ${isEdge ? 'opacity-50' : 'btn border-thirtyDark'}`}>
        {pointer}
      </div>
    );
  };
        ...
        {pages.map(page => {
          ...
          return (
            <div
              ...
              active={isActivePage}
              className={`mt-1 mx-1 h-3 w-3 transition-all rounded-full ${isActivePage ? 'bg-thirtyDark' : 'shadow-emptyPagination hover:bg-thirtyLight'}`}
            />
          );
        })}
      </div>
  ...
  return (
    <div className='w-5/12 h-32 p-1 bg-primary rounded-lg border-2 border-primaryDark border-opacity-40'>
      {photos?.length === 0 ? (
        <div className='w-32 h-full mx-auto flex items-center justify-center bg-primaryDark border-2 border-primary text-center text-primaryLight'>previsualización de imagen</div>
      ) : photos?.length === 1 ? (
