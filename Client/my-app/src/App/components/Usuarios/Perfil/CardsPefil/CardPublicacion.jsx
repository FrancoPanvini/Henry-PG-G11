...
function CardPublicacion({ photo, name, petId, userId, age, size, type, sex, created, onPostPet, found, updated, description, adopted }) {
  ...
  return (
    <div className={`grid grid-cols-10 auto-cols-min place-items-center my-8 rounded-lg py-4 shadow-inner ring ${ringColor} ring-offset-2  w-full h-1/5 border-4 bg-primary text-white`}>
      <div className=''>
      ...
      <div className='col-span-2'>
        <span className=' text-gray-200 text-lg mr-8'>{type ? (adopted ? `Adoptado ${timeAgo}` : `En adopcion ${timeAgo}`) : !type && !found ? `Perdido ${timeAgo}` : `Encontrado ${timeAgo}`}</span>
      </div>
      {type ? (
        ...
        <div className='col-span-3 flex items-center'>
          {!found && (
            <button className='btn bg-gray-200 text-green-600 w-10 h-10 flex justify-center items-center text-2xl mr-2 rounded-lg shadow-inner' title='¡Ya lo encontré!' onClick={handleClick}>
              <BiHappyBeaming />
            </button>
          )}
        </div>
